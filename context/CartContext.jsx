...
export const CartProvider = ({ children }) => {
  ...
  // ── Add to cart ──
  const addToCart = useCallback((productItem, quantity = 1) => {
    try {
      const updatedCart = cartService.addToCart(productItem, quantity);
      setCartItems(prev => {
        const itemId = cartService.getItemKey(productItem);
        const existing = prev.find(p => cartService.getItemKey(p) === itemId);
        const newQty = updatedCart[itemId]?.quantity || 0;
        if (existing) {
          return prev.map(p => cartService.getItemKey(p) === itemId ? { ...p, quantity: newQty } : p);
        }
  ...
  const removeFromCart = useCallback((itemId) => {
    try {
      ...
      setCartItems(prev => prev.filter(item => cartService.getItemKey(item) !== itemId));
  ...
  const updateCartQuantity = useCallback((itemId, quantity) => {
    try {
      const productItem = cartItems.find(p => cartService.getItemKey(p) === itemId);
      if (!productItem) {
      ...
      if (quantity <= 0) {
        ...
        setCartItems(prev =>
          prev.map(item => cartService.getItemKey(item) === itemId ? { ...item, quantity } : item)
        );
  ...
  const saveForLater = useCallback((itemId) => {
    const item = cartItems.find(p => cartService.getItemKey(p) === itemId);
    if (!item) return;

    // Remove from cart
    cartService.removeFromCart(itemId);
    setCartItems(prev => prev.filter(p => cartService.getItemKey(p) !== itemId));

    // Add to saved list (avoid duplicates)
    setSavedItems(prev => {
      if (prev.find(p => cartService.getItemKey(p) === itemId)) return prev;
  ...
  const removeFromSaved = useCallback((itemId) => {
    setSavedItems(prev => {
      const updated = prev.filter(p => cartService.getItemKey(p) !== itemId);
      saveSavedToStorage(updated);
      return updated;
    });
  ...
  const saveItemForLater = useCallback((productItem) => {
    const itemId = cartService.getItemKey(productItem);
    if (!itemId) return;
    setSavedItems(prev => {
      if (prev.find(p => cartService.getItemKey(p) === itemId)) {
  ...
  const moveToCart = useCallback((itemId) => {
    const item = savedItems.find(p => cartService.getItemKey(p) === itemId);
    if (!item) return;
